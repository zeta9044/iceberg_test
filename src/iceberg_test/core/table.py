...
def write_to_table(
    ...
    try:
        ...
        # 테이블 스키마를 PyArrow 스키마로 변환
        pa_schema = schema_to_pyarrow(table.schema())

        # 데이터프레임을 해당 스키마를 가진 PyArrow 테이블로 변환
        arrow_table = pa.Table.from_pandas(df, schema=pa_schema)
...
def read_from_table(
    ...
    try:
        ...
        # 선택할 필드 준비
        selected_fields = tuple(columns) if columns is not None else ("*",)
        ...
        if filter_expr is not None:
            scan_kwargs["row_filter"] = filter_expr
